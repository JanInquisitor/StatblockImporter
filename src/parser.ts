...
//This needs some tweaking to work with the correct npc character format
export interface CharacterNPC {
    ...
    damage?: string;
    baseSave?: number;
    saveModifiers?: string | null;
    morale?: number;
    xp?: number;
    abilities?: AbilityScores;
    special?: string;
    specialAbilitiesList?: string[];
    specialAbilitiesParsed?: SpecialAbility[];
    gear?: string;
    gearList?: string[];
    wealth?: Wealth;
    quantity?: number;
...
//This needs some tweaking to work with the correct npc monster format
export interface Monster {
    name: string;
    subtype?: string | null;
    type: 'monster';
    description?: string;
    numberEncountered?: string;
    numberInLair?: number | string | null;
    alignment?: string;
    size?: string;
    height?: string | null;
    weight?: string | null;
...
export type ParsedStatBlock = CharacterNPC | Monster;
...
function parseCharacterStatBlock(statBlockText: string) {
    ...
    if (alMatch) npc.alignment = alMatch[1];
    ...
    if (faMatch) npc.fightingAbility = parseInt(faMatch[1]);

    const taMatch = statBlockText.match(StatBlockPatterns.turningAbility);
    if (taMatch) npc.turningAbility = parseInt(taMatch[1]);

    const caMatch = statBlockText.match(StatBlockPatterns.castingAbility);
    if (caMatch) npc.castingAbility = parseInt(caMatch[1]);
    ...
    if (mlMatch) npc.morale = parseInt(mlMatch[1]);

    // Extract XP
    const xpMatch = statBlockText.match(StatBlockPatterns.experiencePoints);
    if (xpMatch) npc.xp = parseInt(xpMatch[1].replace(/,/g, ''));
 ...
 */
export function parseMonsterStatBlock(statBlockText: string): Monster {
    const monster: Monster = {name: '', type: 'monster'};
    ...
    // Extract alignment
    const alMatch = statBlockText.match(StatBlockPatterns.alignment);
    if (alMatch) monster.alignment = alMatch[1];
    ...
    if (dxMatch) monster.dexterity = parseInt(dxMatch[1]);

    // Extract AC
    const acMatch = statBlockText.match(StatBlockPatterns.armorClass);
    console.log(acMatch);
    if (acMatch) {
        monster.ac = parseInt(acMatch[1].replace(/[−–—]/g, '-'), 10);
        monster.dr = acMatch[2] ? parseInt(acMatch[2].replace(/[−–—]/g, '-'), 10) : null;
    ...
    // Extract morale
    const mlMatch = statBlockText.match(StatBlockPatterns.morale);
    if (mlMatch) monster.morale = parseInt(mlMatch[1]);
 ...
 */
export function parseSpecialAbilities(abilitiesList: string[]): SpecialAbility[] {
    const parsed: SpecialAbility[] = [];
    ...
    for (const abilityString of abilitiesList) {
        ...
        if (match) {
            ...
            if (ability.name.toLowerCase().includes('heroic fighting')) {
                ...
                if (multiplierMatch) ability.attackMultiplier = parseInt(multiplierMatch[1]);
                if (hdMatch) ability.targetHD = parseInt(hdMatch[1]);
            }
 ...
 */
export function parseSpellList(spellText: string): Spell[][] {
 ...
 */
export function calculateWealth(gearList: string[]): Wealth {
    const wealth: Wealth = {
        copper: 0,
        silver: 0,
        gold: 0,
    ...
    const toGold: Record<string, number> = {
        cp: 0.01,
        sp: 0.1,
        ep: 0.5,
        gp: 1,

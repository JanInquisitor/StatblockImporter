import {ParsedStatBlock, CharacterNPC, Monster} from './parser.js';
 ...
 */
export function mapToFoundryActor(parsedData: ParsedStatBlock): any {
 ...
 */
function mapCharacterToFoundry(npc: CharacterNPC): any {
    const actorData: any = {
    ...
    const fakeActorData: any = {
        ...
        system: {
            ...
            mv: {  // Note: 'mv' not 'movement' in v2.2.1
 ...
 */
export function mapMonsterToFoundry(npc: any): any {
    const actorData: any = {
        name: npc.name || "Imported Character",
 ...
 */
function extractSaveModifier(modifiersString: string | null | undefined, saveType: string): number {
    if (!modifiersString) return 0;
 ...
 */
export function createWeaponItems(npc: CharacterNPC): any[] {
    const weapons: any[] = [];

    if (!npc.damage) return weapons;
 ...
 */
export function createGearItems(npc: CharacterNPC): any[] {
    const items: any[] = [];

    if (!npc.gearList) return items;

    for (const gearItem of npc.gearList) {
        // Skip currency items
        if (gearItem.match(/^\d+\s+(cp|sp|ep|gp|pp)$/i)) continue;
 ...
 */
export async function createFoundryActor(parsedData: ParsedStatBlock): Promise<any> {
    ...
    if ('damage' in parsedData && parsedData.damage) {
        const weapons = createWeaponItems(parsedData as CharacterNPC);
        if (weapons.length > 0) {
    ...
    if ('gearList' in parsedData && parsedData.gearList) {
        const gear = createGearItems(parsedData as CharacterNPC);
        if (gear.length > 0) {
 ...
 */
export async function batchImportActors(parsedDataArray: ParsedStatBlock[]): Promise<any[]> {
    const actors: any[] = [];
